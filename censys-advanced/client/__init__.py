...
__EXECDIR__ = os.path.dirname(os.path.realpath(__file__))
        ...
        try:
            ...
            with open(self.outputFile, 'a+') as file:
                for result in httpResponse['result']['hits']:
                    if len(self.rawData.splitlines()) >= self.count:
                        print(f"The scan is {Main.textHighlight('finished')}, all results has been saved in {Main.textHighlight(self.tempFile)}", end='\r')
                        file.close()
                        return True
                        pass
